mutable struct EpiFastRenyiQKDTri{T<:Real,R<:RealOrComplex{T}} <: Cone{T}
    α::T
    sα::Int
    ...
    function EpiFastRenyiQKDTri{T,R}(
        α::T,
...
use_dder3(cone::EpiFastRenyiQKDTri) = true

function reset_data(cone::EpiFastRenyiQKDTri)
    return (
        cone.feas_updated =
            cone.grad_updated =
                cone.hess_updated =
...
function setup_extra_data!(cone::EpiFastRenyiQKDTri{T,R}) where {T<:Real,R<:RealOrComplex{T}}
    d = cone.d
    Gd = cone.Gd
    Zd = cone.Zd
    ZD = cone.ZD
...
get_nu(cone::EpiFastRenyiQKDTri) = cone.d + 1

function set_initial_point!(arr::AbstractVector{T}, cone::EpiFastRenyiQKDTri{T,R}) where {T<:Real,R<:RealOrComplex{T}}
    d = cone.d
    blocks = cone.blocks
    ShZρ = cone.ShZρ
...
function update_feas(cone::EpiFastRenyiQKDTri)
...
function update_grad(cone::EpiFastRenyiQKDTri)
    @assert cone.is_feas
    blocks = cone.blocks
...
function update_hess_aux(cone::EpiFastRenyiQKDTri)
...
function hess_prod!(prod::AbstractVecOrMat, arr::AbstractVecOrMat, cone::EpiFastRenyiQKDTri)
...
function update_hess(cone::EpiFastRenyiQKDTri)
    cone.hess_aux_updated || update_hess_aux(cone)
...
function update_dder3_aux(cone::EpiFastRenyiQKDTri)
...
function dder3(cone::EpiFastRenyiQKDTri{T,R}, dir::AbstractVector{T}) where {T<:Real,R<:RealOrComplex{T}}
